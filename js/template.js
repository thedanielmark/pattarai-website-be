...
$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: "https://www.instagram.com/licetpattarai/?__a=1",
        datatype: "html",
        success: function (response) {
            ...
            for(var i = 0; i < 4; i++) {
                ...
                var date = timeConverter(response.graphql.user.edge_owner_to_timeline_media.edges[i].node.taken_at_timestamp);
                var fullCaption = response.graphql.user.edge_owner_to_timeline_media.edges[i].node.edge_media_to_caption.edges[0].node.text;
                ...
                //  Setting thumbnail
                var thumbnail = response.graphql.user.edge_owner_to_timeline_media.edges[i].node.thumbnail_src;
                document.getElementById("post" + i).setAttribute("style", "background-image: url('" + thumbnail + "');");

                // Setting href
                var href = "https://instagram.com/p/" + response.graphql.user.edge_owner_to_timeline_media.edges[i].node.shortcode;
    ...
    $("#submitNewsletter").click(function() {
        ...
        if (email === "") {
            ...
            $.ajax({
                type: "POST",
                url: "https://mx-out-mixspace.thedanielmark.com/subscribe-newsletter-pattarai.php",
                datatype: "html",
                data: {
                    email: email,
                },
